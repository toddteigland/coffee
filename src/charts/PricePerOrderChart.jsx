...
export default function PricePerOrderChart() {
  ...
  async function fetchPricePerOrder() {
    const response = await fetch(`http://localhost:8080/pricePerOrder?storeId=${store.id}`);
    ...
    <div className={styles.ppoChartContainer}>
      <LineChart width={1000} height={500} data={priceData}>
        ...
        <XAxis dataKey="order_id">
          {/* <Label value="Order Number" angle={0} position="Bottom" offset={-10} /> */}
        </XAxis>
        <YAxis dataKey="order_total">
          <Label value="Price in $" angle={-90} position="insideLeft" offset={10} />
        </YAxis>
        <Tooltip />
        <Line type="monotone" dataKey="order_total" stroke="#8d6d6d" name="Price per Order" />
        <Line type="monotone" dataKey="runningAverage" stroke="#0000ff" name="Running Average Price" dot={false} />
        <Legend />
      </LineChart>
    </div>
  );
